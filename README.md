...
여행 일정을 정할 때, 결정은 보통 자녀 한 명에게 몰립니다.

SAMSAM55는 자녀가 일정 후보(선택지)를 준비하면 부모님이 밸런스 게임처럼 골라서, 여행 일정을 "함께의 결정"으로 만드는 서비스입니다.
...
- 정적 자원(프론트엔드 빌드 산출물)은 S3에 올리고 CloudFront로 서빙합니다.
- API 요청은 퍼블릭 서브넷 EC2의 nginx가 받아 Spring Boot 컨테이너로 리버스 프록시합니다.
- MySQL은 프라이빗 서브넷의 별도 EC2에서 Docker로 띄워 외부에서 직접 접근할 수 없습니다.
- 선택지 사진은 Spring Boot가 발급한 presigned URL로 클라이언트가 S3에 직접 업로드합니다.
...
부모님은 회원가입 없이 초대 링크로 들어와 세션과 복구용 쿠키로만 식별됩니다. 브라우저 쿠키를 지우거나 다른 기기로 들어오면 이미 투표한 이력을 이어갈 수 없습니다. 기기 고유 식별자 등으로 보완하는 방법을 검토했지만, 로그인 없는 접근성을 우선하기로 하고 이번 범위에서는 현재 방식을 유지하기로 했습니다.

**확정 전 일정의 재투표 허용**

참여자가 투표를 다시 제출하면 이전 투표를 덮어씁니다. 최신 요구사항 문서에는 "기록된 투표는 변경할 수 없다"는 조항이 있어, 이 부분은 팀 논의를 거쳐 정책을 다시 확정해야 합니다.

**선택지 설명 자동 생성(AI) 미구현**

선택지 설명을 AI가 자동으로 채워주는 기능은 아직 스텁 상태입니다. 실제 LLM 연동 전이라 항상 빈 값을 반환하며, 방장이 직접 입력한 설명만 화면에 노출됩니다.

---

## 개발자 조 구성원 정보

소프티어 부트캠프 8기 해커톤 · SAMSAM55
